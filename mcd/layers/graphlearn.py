...
import numpy as np
from mcd.utils.generic_utils import to_cuda
from mcd.src.constants import INF
...
class GraphLearner(nn.Module):
    def __init__(self, input_size, hidden_size, topk=None, epsilon=None, num_pers=16, metric_type='gat_attention', device=None):
        ...
        if metric_type == 'attention':
            self.linear_sims = nn.ModuleList([nn.Linear(input_size, hidden_size, bias=False) for _ in range(num_pers)]) #专门用于存储module的list  输出其中除了最后一个维度之外的所有维度都与输入的形状相同，
            print('[ Multi-perspective {} GraphLearner: {} ]'.format(metric_type, num_pers))

        elif metric_type == 'weighted_cosine':
            self.weight_tensor = torch.Tensor(num_pers, input_size)
            self.weight_tensor = nn.Parameter(nn.init.xavier_uniform_(self.weight_tensor))   # 是一个服从均匀分布的Glorot初始化器，也就是把权重矩阵先随机初始化
            print('[ Multi-perspective {} GraphLearner: {} ]'.format(metric_type, num_pers))
    ...
    def forward(self, context, ctx_mask=None):
        ...
        Returns
        :attention, (batch_size, ctx_size, ctx_size)  是计算的相似性矩阵
        """
        ...
        elif self.metric_type == 'dot_atten':
            weight_martix = torch.rand_like(context)
            context_fc = torch.mul(context, weight_martix)
            context_fc1 = context
            attention = torch.matmul(context_fc1, context_fc.transpose(-1, -2))
        ...
        if ctx_mask is not None:
            attention = attention.masked_fill_((1 - ctx_mask.byte().unsqueeze(1)).bool(), markoff_value)
            attention = attention.masked_fill_((1 - ctx_mask.byte().unsqueeze(-1)).bool(), markoff_value)
...
def get_binarized_kneighbors_graph(features, topk, mask=None, device=None):  # 传入的是raw context  按批处理的
