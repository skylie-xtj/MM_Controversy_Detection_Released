...
from .base_module import Linear, LayerNormalization, PosEncoding, ScaledDotProductAttention
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_k, d_v, n_heads, dropout, d_model, video_len, asr_len, fea_v, fea_a, pos):
        ...
        self.pos_emb_v = PosEncoding(video_len * 10, d_model)
        self.pos_emb_s = PosEncoding(asr_len * 10, d_model)
        self.linear_v = nn.Linear(in_features=fea_v, out_features=d_model)
        self.linear_s = nn.Linear(in_features=fea_a, out_features=d_model)
    ...
    def forward(self, v, a, v_len, a_len):
        b_size = v.size(0)
        # q: [b_size x len_q x d_model]
        # k: [b_size x len_k x d_model]
        # v: [b_size x len_v x d_model] note (len_k == len_v)
        v, a = self.linear_v(v), self.linear_s(a)
        if self.pos:
            pos_v, pos_s = self.pos_emb_v(v_len), self.pos_emb_s(a_len)
            residual_v, residual_s = v + pos_v, a + pos_s
        else:
            residual_v, residual_s = v, a
        # context: a tensor of shape [b_size x len_q x n_heads * d_v]
        q_v, k_v, v_v = self.multihead_attn_v(v, v, v)
        q_s, k_s, v_s = self.multihead_attn_s(a, a, a)
        ...
        t = residual_s + output_s
        return self.layer_norm_v(residual_v + output_v), self.layer_norm_s(t)
...
class Co_Attention(nn.Module):
    def __init__(self, d_k, d_v, n_heads, dropout, d_model, video_len, asr_len, fea_v, fea_a, pos):
        ...
        self.multi_head = MultiHeadAttention(d_k=d_k, d_v=d_v, n_heads=n_heads, dropout=dropout, d_model=d_model,
                                             video_len=video_len, asr_len=asr_len, fea_v=fea_v, fea_a=fea_a, pos=pos)
    ...
    def forward(self, v, a, v_len, a_len):
        v, a = self.multi_head(v, a, v_len, a_len)
        v = self.PoswiseFeedForwardNet_v(v)
        a = self.PoswiseFeedForwardNet_s(a)
        return v, a
